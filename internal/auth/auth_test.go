...
func TestConfigTalksToThePublicClientEndpoints(t *testing.T) {
	conf := Config("https://atm.example", "http://127.0.0.1:4711/callback")
	...
	assert.Equal(t, "https://atm.example/api/v1/oauth/authorize", conf.Endpoint.AuthURL)
	assert.Equal(t, "https://atm.example/api/v1/oauth/token", conf.Endpoint.TokenURL)
}
